...
- **Semantic review**: Performed by the active agent; no external provider is selected automatically
...
- "check if this skill is ready to share"
...
- **`script-bearing`** — has executable scripts under `scripts/` (`.sh`, `.py`, `.js`, `.ts`, `.mjs`).
  ...
  applies (Security, Error Handling, README, Script Quality, etc.). Raw score ceiling: **125**.
...
### Step 3: Semantic Review (Active Agent)

The active agent reviews the skill content to:
...
Do not automatically select or invoke an external provider. If an external reviewer
is explicitly requested, confirm the current runtime policy and the exact diff/data
scope before sending anything.
...
**Executive Summary:**
- Overall score (0-100)
- Readiness: Ready, Conditional, or Not ready
...
**Semantic review**: Active agent; external review requires explicit authorization
```
...
**Total**: dynamic — raw check ceiling is 125 points for `script-bearing` and 65 for `pure-instruction` (inapplicable checks like Security/Error Handling sections, Security keyword density, README, and Script Quality are excluded). The reported score is always normalized to 0–100.
...
**Readiness (independent from score):**
...
1. **Test on High-Quality Skill**
   - Input: interactive-presentation-generator (expected normalized score: 75+)
   - Expected: 0 critical and 0 important issues, readiness `Ready`

2. **Test on Problematic Skill**
   - Input: Skill with hardcoded paths, missing security section
   - Expected: 2+ critical issues, readiness `Not ready`

3. **Self-Audit**
   - Input: skill-auditor itself
   - Expected: normalized score and readiness agree with the reported findings
...
- Automated checks complete in < 5 seconds; semantic review is performed by the active agent
- External reviewers require an explicit request, current runtime-policy check, and approved diff/data scope
...
### For Teams
1. Use the normalized score for quality reporting, not as the sole release gate
2. Require `Ready` status before sharing or merging
3. Use `Conditional` for explicit, documented exceptions only
