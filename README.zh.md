...
| [skill-sync](./skill-sync/) | 將 `~/.claude/skills/` 單向**鏡射**到其他 agent skill 資料夾（codex、gemini、cursor、antigravity）。使用 `rsync --delete`：target 端有但 source 端沒有的檔案**會被刪除**，會在 dry-run 預覽明確列出，之後再透過 `y/N` 確認（預設 N）。支援自訂 target 和 exclude 清單，預設值含 `spec-generator` 的 symlink 保護。無外部依賴。 |
