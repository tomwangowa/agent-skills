...
- **Pre-commit auto-review**: 當準備提議 commit 之前（例如「要我 commit 嗎？」「是否要提交？」「ready to commit」），MUST 先主動調用 `code-review-claude`。review 完成後再詢問使用者是否 commit。只有在該次變更已經跑過 Claude native review 的情況下才跳過。`code-review-gemini` 已退役，不得被路由或呼叫、附加到 pre-commit，或接收 diff；未來 RDSec endpoint reviewer 必須由使用者明確選擇模型並確認可送出的 diff／資料範圍。
